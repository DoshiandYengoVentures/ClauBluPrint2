(function () {
  ...
  ready(function () {
    ...
    /* ---------- Hero: line-reveal headline on load ---------- */
    ...
    if (hero) {
      ...
      /* ---------- Hero: cursor coordinate readout (signature interaction) ---------- */
      var crosshair = hero.querySelector(".hero-crosshair");
      var coord = hero.querySelector(".hero-coord");
      if (crosshair && coord && window.matchMedia("(hover: hover)").matches) {
        hero.addEventListener("mouseenter", function () { hero.classList.add("is-tracking"); });
        hero.addEventListener("mouseleave", function () { hero.classList.remove("is-tracking"); });
        hero.addEventListener("mousemove", function (e) {
          var rect = hero.getBoundingClientRect();
          var x = e.clientX - rect.left;
          var y = e.clientY - rect.top;
          crosshair.style.transform = "translate(" + x + "px," + y + "px)";
          coord.style.transform = "translate(" + (x + 14) + "px," + (y - 4) + "px)";
    ...
    /* ---------- Expandable rows (services spec-list + FAQ share this pattern) ---------- */
    document.querySelectorAll(".spec-row").forEach(function (item) {
      var btn = item.querySelector(".spec-row-head");
      var panel = item.querySelector(".spec-body");
      if (!btn || !panel) return;
      btn.addEventListener("click", function () {
        var isOpen = item.getAttribute("data-open") === "true";
        item.parentElement.querySelectorAll(".spec-row").forEach(function (other) {
          other.setAttribute("data-open", "false");
          other.querySelector(".spec-body").style.maxHeight = null;
        });
        if (!isOpen) {
    ...
    /* ---------- Contact form: validate, then submit to Groq-backed endpoint's neighbor form service ---------- */
